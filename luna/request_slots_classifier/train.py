...
class RequestSlotsClassifierSystem(pl.LightningModule):
    def __init__(self, labels):
        ...
        self.accuracy = torchmetrics.Accuracy(
            task="multilabel", num_labels=len(labels))
    ...
    def configure_optimizers(self):
        ...
        optimizer_grouped_parameters = [
            {'params': [p for n, p in self.request_slots_classifier.model_transformers.named_parameters() if not any(nd in n for nd in no_decay)],
             'weight_decay': self.weight_decay},
            {'params': [p for n, p in self.request_slots_classifier.model_transformers.named_parameters(
...
torch.save(check_point['state_dict'],
           'checkpoints/request_slots_classifier.bin')
