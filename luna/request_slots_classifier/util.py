    ...
    for dialogue_data in raw["dialogue_data"]:
        if dialogue_data["domain"] == "fashion":
            for dialogue in dialogue_data["dialogue"]:
                request_slots = dialogue["transcript_annotated"]["act_attributes"][
                    "request_slots"
                ]
                if (
                    dialogue["transcript_annotated"]["act"] == "ASK:GET"
                    or dialogue["transcript_annotated"]["act"] == "REQUEST:COMPARE"
                ) and len(request_slots) > 0:
                    rs.append(
                        {"text": dialogue["transcript"], "request_slots": request_slots}
                    )
