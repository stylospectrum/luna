...
def get_fashion_dataset():
    ...
    rs = []
    intent_map = {"REQUEST:GET": "buy", "ASK:GET": "ask", "REQUEST:COMPARE": "compare"}

    for dialogue_data in raw["dialogue_data"]:
        if dialogue_data["domain"] == "fashion":
            for dialogue in dialogue_data["dialogue"]:
                if (
                    dialogue["transcript_annotated"]["act"] == "REQUEST:GET"
                    or dialogue["transcript_annotated"]["act"] == "ASK:GET"
                    or dialogue["transcript_annotated"]["act"] == "REQUEST:COMPARE"
                ):
                    rs.append(
                        {
                            "text": dialogue["transcript"],
                            "intent": intent_map[
                                dialogue["transcript_annotated"]["act"]
                            ],
                        }
                    )
    return rs
...
def get_input_examples():
    ...
    chit_chat_dataset["intent"] = [
        "other" for _ in range(len(chit_chat_dataset["Question"]))
    ]
