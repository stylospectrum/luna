...
def get_raw_slot_labels():
    slots = [
        dialogue["transcript_annotated"]["act_attributes"]["slot_values"]
...
def process_slot_value(words, slot_label, slot_value):
    ...
    if slot_label == "price" and slot_value == "expensive":
        if set(["pricey"]).issubset(set(words)):
            return "pricey"
        if set(["high", "price"]).issubset(set(words)) and not set(
            ["high", "in", "price"]
        ).issubset(set(words)):
            return "high price"
    ...
    if (
        set(["extra", "small"]).issubset(set(words))
        and slot_label == "size"
        and (slot_value == "XS" or slot_value == "xs")
    ):
        return "extra small"

    if (
        set(["small"]).issubset(set(words))
        and not set(["extra"]).issubset(set(words))
        and slot_label == "size"
        and (slot_value == "s" or slot_value == "S")
    ):
        return "small"

    if (
        set(["medium"]).issubset(set(words))
        and slot_label == "size"
        and (slot_value == "M" or slot_value == "m")
    ):
        return "medium"

    if (
        set(["large"]).issubset(set(words))
        and not set(["extra"]).issubset(set(words))
        and slot_label == "size"
        and (slot_value == "l" or slot_value == "L")
    ):
        return "large"

    if (
        set(["extra", "large"]).issubset(set(words))
        and slot_label == "size"
        and (slot_value == "XL" or slot_value == "xl")
    ):
        return "extra large"

    if (
        set(["shirt"]).issubset(set(words))
        and slot_label == "type"
        and slot_value == "blouse"
    ):
        return "shirt"

    if (
        set(["pant"]).issubset(set(words))
        and slot_label == "type"
        and (
            slot_value == "trousers" or slot_value == "jeans" or slot_value == "joggers"
        )
    ):
        return "pant"

    if (
        set(["affordably", "price"]).issubset(set(words))
        and slot_label == "price"
        and slot_value == "affordable"
    ):
        return "affordably"

    if (
        set(["budget", "friendly"]).issubset(set(words))
        and slot_label == "price"
        and (slot_value == "affordable" or slot_value == "cheap")
    ):
        return "budget friendly"

    if (
        set(["gray"]).issubset(set(words))
        and slot_label == "color"
        and slot_value == "grey"
    ):
        return "gray"

    if (
        set(["long", "sleeve"]).issubset(set(words))
        and slot_label == "sleeveLength"
        and slot_value == "full"
    ):
        return "long"
    ...
    for dialogue_data in raw_data["dialogue_data"]:
        if dialogue_data["domain"] == "fashion":
            for dialogue in dialogue_data["dialogue"]:
                if dialogue["transcript_annotated"]["act"] == "REQUEST:GET":
                    s = ""
                    slots = dialogue["transcript_annotated"]["act_attributes"][
                        "slot_values"
                    ]
