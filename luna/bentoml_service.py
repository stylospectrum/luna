...
async def predict(message: MessageModel) -> dict[str, str]:
    ...
    user_preferences = (
        {} if "user_preferences" not in info else json.loads(info["user_preferences"])
    )
    intent = await classifier_runner.classify.async_run(content, "intent")

    if (
        intent in not_intent_buy_labels and (context == "" or context is None)
    ) or intent == "add_to_cart":
...
async def get_embeddings(texts: Series):
    ...
    embeddings = await classifier_runner.classify.async_run(
        texts, "intent", "get_embeddings"
    )
    return embeddings.numpy()


@svc.api(input=JSON(pydantic_model=ClassificationParams), output=JSON())
async def classify(params: ClassificationParams):
    params = params.model_dump()
    text: str = params["text"]
    type: str = params["type"]

    if type == "intent":
        intent = await classifier_runner.classify.async_run(text, "intent")
        return {"intent": intent}

    return await classifier_runner.classify.async_run(text, type)
