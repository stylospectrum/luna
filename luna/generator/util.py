...
def featurize(question_id, answer_id, tokenizer, with_eos=True):
    ...
    sequence = (
        [[bos]] + [question_id] + [[system] + answer_id + ([eos] if with_eos else [])]
    )
    sequence = (
        [sequence[0]]
        + [
            [user if (len(sequence) - i) % 2 else system] + s
    ...
    instance["token_type_ids"] = [
        user if i % 2 or i == 0 else system for i, s in enumerate(sequence) for _ in s
    ]
    instance["labels"] = (
...
def get_features():
    ...
    for dialogue_data in fashion_raw["dialogue_data"]:
        if dialogue_data["domain"] == "fashion":
            for dialogue in dialogue_data["dialogue"]:
                if (
                    dialogue["transcript_annotated"]["act"] == "REQUEST:ADD_TO_CART"
                    and dialogue["system_transcript_annotated"]["act"]
                    == "CONFIRM:ADD_TO_CART"
                ):
                    fashion_data.append(
                        {
    ...
    bitext_df = pd.read_csv(
        "Bitext_Sample_Customer_Support_Training_Dataset_27K_responses-v11.csv"
    )
    bitext_df = bitext_df.rename(
    ...
    features = [
        ...
        for f in [
            featurize(
                tokenizer.encode(row["Question"]),
                tokenizer.encode(row["Answer"]),
                tokenizer,
            )
    ...
    for feature in features:
        ...
        for name in feature.keys():
            temp = feature[name] + [padding_id if name != "labels" else -100] * (
                max_l - len(feature[name])
            )
