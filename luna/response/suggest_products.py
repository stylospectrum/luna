...
def suggest_products(
    slot: dict[str, str], products: list[dict[str, str]], active="suggest"
):
    suggested_product = []
    ...
    customer_review_request = (
        slot["customerReview"] if "customerReview" in slot else None
    )
    ...
    number_of_products = 5 if len(suggested_product) > 5 else len(suggested_product)
    number_of_products = number_of_products if active == "suggest" else 1

    responses = (
        template_response[str(number_of_products)]
        if active == "suggest"
        else [user_ask_response]
    )
        ...
        if product["type"] in template_product_benefit:
            if "unique_feature/benefit" in responses[idx]:
                responses[idx] = responses[idx].replace(
                    "[unique_feature/benefit]",
                    template_product_benefit[product["type"]]["unique_feature/benefit"],
                )

            if "occasion/usage" in responses[idx]:
                responses[idx] = responses[idx].replace(
                    "[occasion/usage]",
                    template_product_benefit[product["type"]]["occasion/usage"],
                )
